...
export class IdeaService {
  ...
  async showAll(page: number = 1, newest: boolean = false): Promise<IdeaRO[]> {
    ...
    return ideas.map(idea => {
      return this.toResponseObjectGen(idea);
    });
  }
  ...
  async update(
    ...
    // 'Partial' dont expect entire object
  ...
  public toResponseObject(idea: Idea): IdeaRO {
    ...
    if (idea.comments.length >= 1) {
      if (checkForComments.length > 2) {
        const comments = idea.comments.map(
          (comment): CommentRO => {
  ...
  public toResponseObjectGen(idea: Idea): IdeaRO {
    let author;
    const checkForComments = idea.comments.toString().split(':');
    ...
    if (idea.comments.length >= 1) {
      if (checkForComments.length > 2) {
        // const comments = idea.comments.map(
        //   (comment): CommentRO => {
        //     comment.populate('-password').execPopulate();
